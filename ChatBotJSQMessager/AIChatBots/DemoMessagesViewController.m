...
#pragma mark start coreMotion
-(void)startCoreMotionManager
{
...
if ([self.motionManager isGyroAvailable]) {
    [self.motionManager startGyroUpdatesToQueue:[NSOperationQueue mainQueue]
                                    withHandler:^(CMGyroData *gyroData, NSError *error) {
                                        NSLog(@"X:%@,Y:%@,Z:%@",[NSString stringWithFormat:@"%.3f",
                                                                 gyroData.rotationRate.x],[NSString stringWithFormat:@"%.3f",
                                                                                           gyroData.rotationRate.y],[NSString stringWithFormat:@"%.3f",
    ...
    [self.motionManager startDeviceMotionUpdatesToQueue:
     [NSOperationQueue mainQueue]
                                            withHandler:^(CMDeviceMotion *motion, NSError *error) {
                                                ...
                                                NSLog(@"Yaw:%@,Pitch:%@,Roll:%@",[NSString stringWithFormat:@"%.3f",
                                                                                  degrees(attitude.yaw)],[NSString stringWithFormat:@"%.3f",
                                                                                                          degrees(attitude.pitch)],[NSString stringWithFormat:@"%.3f",
                                                                                                                                    degrees(attitude.roll)]);
                                            }];
}
}

#pragma mark start iBeacon
